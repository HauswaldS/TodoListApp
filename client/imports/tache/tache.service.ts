...
export class TacheService {
    ...
    updateDossierTaches(ownerId){
        let dossierTaches = this.findDossierTaches(ownerId);
        this.dossierService.addTachesToDossier(dossierTaches, ownerId);
        console.log(dossierTaches);
    }

    deleteDossierTaches(ownerId) {
        let taches = this.taches.find({ dossierId: ownerId }).fetch();
    ...
    changeStatus(tacheId) {
        let tacheStatus: boolean = this.taches.find({ _id: tacheId }).fetch()[0].status;
        if (tacheStatus === true) {
            this.taches.update(
                { _id: tacheId },
                {
